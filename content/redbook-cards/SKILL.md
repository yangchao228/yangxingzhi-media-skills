...
本技能负责“小红书 HTML 卡片预览”。如果用户需要逐页图片生成提示词、7/8/9 页结构判断、发布配文和评论区引导，优先使用 `xiaohongshu-viral-image-skill-v4`。如果用户需要公众号方形配图，使用 `wechat-to-cards`。
...
本技能优先提供低成本 HTML 预览，不默认升级到逐页图片生成。

- 先给卡片数量、结构和每张摘要；用户确认后再输出完整 HTML。
- 原文很短时优先 5 张以内，不为了显得完整而凑到 7 张。
- 只改标题、标签、某张卡片或配色时，局部替换，不重新生成全套 HTML。
- 用户明确要真实图片集合时，再转 `xiaohongshu-viral-image-skill-v4`。
- 不在未确认方向时生成多套风格变体。
