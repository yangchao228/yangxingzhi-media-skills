...
- 文昌总控、路由和卡片类 skill 的成本敏感执行规则

## 成本敏感工作方式

`文昌.skill` 默认按成本敏感方式运行：先判断范围，再生成成品；先复用结构化状态，再扩写多平台资产。

执行时遵守这些规则：

- 用 `content_state` 和 `handoff` 接力，不把整段聊天历史传给下一阶段。
- 选题、标题、平台、页数、视觉风格、封面、上传、归档等关键节点先确认。
- 卡片/图片任务先给页数、结构和视觉规范，再生成 HTML、PNG、出图提示词或上传。
- 返工时优先局部修改，不整篇重跑。
- 涉及订阅、模型价格、生产配置、外部上传和真实发布时，只给建议，不替用户拍板。
...
Human3.0 成书归档维护在 `human3.0_book/`。只有完成成书守门员审查，并由用户确认进入归档的内容，才进入这个目录；`content/outputs/` 仍用于保存具体内容产物。
